...
Todo el estado se mantiene en un único objeto `state` que actúa como *store* ligero:
...
function speak(text) {
  ...
  utt.lang = 'es-ES';
  utt.onstart = () => { state.speaking = true; setAssistantState('speaking'); };
  utt.onend = () => {
...
async function setupVisionEngines() {
  ...
  state.faceMesh.onResults((r) => drawFrame(r.image, null, r.multiFaceLandmarks));
...
async function handleVoiceCommand(rawText) {
  ...
  if (text.includes('modo normal'))  { setMode('normal'); speak('Modo normal');         return; }
  ...
  if (text.includes('detener cámara') || text.includes('parar cámara')) {
    await stopCamera(); speak('Cámara detenida'); return;
  }
  ...
  if (text.startsWith('guardar nota')) {
    ...
    if (note) { await addNote(note); speak('Nota guardada'); } return;
  }
  if (text.includes('leer notas'))   { await readNotes(); return; }
  if (text.includes('eliminar notas')) {
    nousConfirm('¿Eliminar todas las notas?').then(async (ok) => {
      if (ok) { await clearStore('notes'); renderNotes(); speak('Notas eliminadas'); }
    });
    return;
  }
}
...
<nav class="tabs" role="tablist">
  <button class="tab active" data-tab="main" role="tab" aria-selected="true">Principal</button>
...
.tab {
  ...
  background: none; color: var(--muted); cursor: pointer; margin-bottom: -2px;
}
...
.confidence-fill { height: 100%; border-radius: 10px; transition: width .3s ease; }
...
.camera-placeholder {
  ...
  font-size: 1rem; color: #94a3b8; background: #0f172a; z-index: 2;
}
...
.badge-listening { background: #dbeafe; border-color: var(--blue); }
.badge-listening .badge-dot { background: var(--blue); animation: pulse 1.5s infinite; }
.badge-speaking { background: #dcfce7; border-color: var(--green); }
.badge-speaking .badge-dot { background: var(--green); }
.badge-error { background: #fef2f2; border-color: var(--red); }
...
function initDarkMode() {
  ...
  el.darkModeBtn?.addEventListener('click', () => {
    ...
    showToast(
      document.body.classList.contains('dark') ? 'Modo oscuro activado' : 'Modo claro activado',
      'info'
    );
  });
}
...
body.dark {
  ...
  --muted: #9ca3af; --border: #3a3d42; --border-strong: #4b4f56; --accent: #e4e5e7;
}
```
...
function showToast(msg, tone = 'info') {
  ...
  div.innerHTML = '<span class="toast-icon">' + (icons[tone] || '') + '</span> ' + escapeHtml(msg);
...
.toast {
  ...
  pointer-events: auto; box-shadow: 0 4px 12px rgba(0,0,0,.15);
}
...
function nousConfirm(message) {
  return new Promise((resolve) => {
    ...
    const close = (val) => { overlay.hidden = true; resolve(val); };
...
*Documento generado para la actividad de Desarrollo de Interfaces — DAM2 2025/26.*
